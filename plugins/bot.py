...
URL_SEND_VIDEO = "https://api.telegram.org/bot{}/sendVideo?chat_id={}"
...
class MessagingBot:
    ...
    def send_video_bytes(self, chat_id: int,  video_bytes: bytes) -> RespMessage:
        payload = {}
        headers = {}
        files = [
            ('video', ('video.mp4', video_bytes, 'video/mpeg'))
        ]
        response = req.get(URL_SEND_VIDEO.format(self.token, chat_id), headers=headers, data=payload, files=files)
